...
function extractStr($filter_content_arr, $content){
	echo  "<meta charset='UTF-8' /><br>".__FILE__.' line:'.__LINE__.'============'."<pre>";print_r($filter_content_arr);exit;
	$pos_start = strpos($content, $filter_content_arr['start_str']);
		$pos_end = strpos($content, $filter_content_arr['end_str']);
...
function getReplaceResult($filter_content_arr, $content, $is_debug=''){
					...
					if(isset($filter_content_arr['use_extract'])){
						$content = $this->extractStr($filter_content_arr, $this->html_content);
					}else if(isset($filter_content_arr['use_minus'])){
						$content = $this->minusStr($filter_content_arr, $this->html_content);
					}else{
						$content = $this->pregReplaceStr($filter_content_arr, $this->html_content);
					}
		/**
	if($is_debug=='desc'){
