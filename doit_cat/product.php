  ...
  if (zen_not_null($action)) {
    switch ($action) {
      ...
      case 'update_product_meta_tags':
        ...
        break;
		case 'save_labels':
      	//echo DIR_WS_MODULES . $zc_products->get_handler($product_type) . '/save_labels.php';
      	require(DIR_WS_MODULES . $zc_products->get_handler($product_type) . '/save_labels.php');
        break;
      case 'insert_product':
      case 'update_product':
...
<?php
  if ($action == 'new_product' or $action == 'new_product_meta_tags') {
  ...
  } elseif ($action== 'modify_labels' or $action== 'tuijian'  or $action== 'tuijian2'){
	...
	if($action== 'tuijian2')
	{
		...
		//print_r($_GET);
		$pid=$_GET[pID];
		mysql_query("delete from products_recommend where products_id='$pid'");

		mysql_query("INSERT INTO  `products_recommend` (`id` ,`products_id` ,`time`)VALUES (NULL ,  '$pid', now());");
    ...
    require(DIR_WS_MODULES . 'product_labels_modify.php');
