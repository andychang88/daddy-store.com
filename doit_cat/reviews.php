  ...
  if (zen_not_null($action)) {
    switch ($action) {
      ...
      case 'update':
		...
		$price_rating = zen_db_prepare_input($_POST['price_rating']);
		$value_rating = zen_db_prepare_input($_POST['value_rating']);
		$quality_rating = zen_db_prepare_input($_POST['quality_rating']);
		
        $reviews_text = zen_db_prepare_input($_POST['reviews_text']);

        $db->Execute("update " . TABLE_REVIEWS . "
                      set price_rating = '" . zen_db_input($price_rating) . "',
					      value_rating = '" . zen_db_input($value_rating) . "',
						  quality_rating = '" . zen_db_input($quality_rating) . "',
...
<script language="javascript" src="includes/reivew_quick_verify.js"></script>
...
<?php
  if ($action == 'edit') {
    ...
    $reviews = $db->Execute("select r.reviews_id, r.products_id, r.customers_name, r.date_added,
									...
									r.quality_rating,r.price_rating,r.value_rating 
...
?>
      ...
      <tr>
        <td class="main"><b><?php echo ENTRY_RATING; ?></b>&nbsp;<?php echo TEXT_BAD; ?>&nbsp;<?php for ($i=1; $i<=5; $i++) echo zen_draw_radio_field('price_rating', $i, '', $rInfo->price_rating) . '&nbsp;'; echo TEXT_GOOD; ?></td>
      </tr>
	  <tr>
        <td class="main"><b><?php echo ENTRY_RATING; ?></b>&nbsp;<?php echo TEXT_BAD; ?>&nbsp;<?php for ($i=1; $i<=5; $i++) echo zen_draw_radio_field('value_rating', $i, '', $rInfo->value_rating) . '&nbsp;'; echo TEXT_GOOD; ?></td>
      </tr>
	  <strong><tr>
        <td class="main"><b><?php echo ENTRY_RATING; ?></b>&nbsp;<?php echo TEXT_BAD; ?>&nbsp;<?php for ($i=1; $i<=5; $i++) echo zen_draw_radio_field('quality_rating', $i, '', $rInfo->quality_rating) . '&nbsp;'; echo TEXT_GOOD; ?></td>
      </tr></strong>
      <tr>
        <td><?php echo zen_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
      </tr>
...
<?php
    ...
    if (zen_not_null($_POST)) {
      ...
      $reviews = $db->Execute("select r.reviews_id, r.products_id, r.customers_name, r.date_added,
                                      r.last_modified, r.reviews_read, rd.reviews_text,
                                      r.price_rating,r.value_rating,r.quality_rating
                               from " . TABLE_REVIEWS . " r, " . TABLE_REVIEWS_DESCRIPTION . " rd
...
?>
      ...
      <tr>
        <td class="main"><b><?php echo ENTRY_RATING; ?></b>&nbsp;<?php echo zen_image(DIR_WS_TEMPLATE_IMAGES . 'stars_' . $rInfo->price_rating . '.gif', sprintf(TEXT_OF_5_STARS, $rInfo->price_rating)); ?>&nbsp;<small>[<?php echo sprintf(TEXT_OF_5_STARS, $rInfo->price_rating); ?>]</small></td>
      </tr>
	  <tr>
        <td class="main"><b><?php echo ENTRY_RATING; ?></b>&nbsp;<?php echo zen_image(DIR_WS_TEMPLATE_IMAGES . 'stars_' . $rInfo->value_rating . '.gif', sprintf(TEXT_OF_5_STARS, $rInfo->value_rating)); ?>&nbsp;<small>[<?php echo sprintf(TEXT_OF_5_STARS, $rInfo->value_rating); ?>]</small></td>
      </tr>
	  <tr>
        <td class="main"><b><?php echo ENTRY_RATING; ?></b>&nbsp;<?php echo zen_image(DIR_WS_TEMPLATE_IMAGES . 'stars_' . $rInfo->quality_rating . '.gif', sprintf(TEXT_OF_5_STARS, $rInfo->quality_rating)); ?>&nbsp;<small>[<?php echo sprintf(TEXT_OF_5_STARS, $rInfo->quality_rating); ?>]</small></td>
      </tr>
      <tr>
        <td><?php echo zen_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
      </tr>
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent" align="right"><?php echo 'Price';//TABLE_HEADING_RATING; ?></td>
				<td class="dataTableHeadingContent" align="right"><?php echo 'Value';//TABLE_HEADING_RATING; ?></td>
				<td class="dataTableHeadingContent" align="right"><?php echo 'Quality';//TABLE_HEADING_RATING; ?></td>
...
//    $reviews_query_raw = "select reviews_id, products_id, date_added, last_modified, reviews_rating, status from " . TABLE_REVIEWS . " order by date_added DESC";
    ...
    while (!$reviews->EOF) {
      if ((!isset($_GET['rID']) || (isset($_GET['rID']) && ($_GET['rID'] == $reviews->fields['reviews_id']))) && !isset($rInfo)) {
        $reviews_text = $db->Execute("select r.reviews_read, r.customers_name,rd.reviews_text,
        ...
        $reviews_average = $db->Execute("select (avg(price_rating) / 5 * 100) as average_rating
                                         from " . TABLE_REVIEWS . "
                                         where products_id = '" . (int)$reviews->fields['products_id'] . "'");
...
?>
     <tr id="defaultSelected" class="<?php echo (isset($rInfo) && is_object($rInfo) && ($reviews->fields['reviews_id'] == $rInfo->reviews_id))? 'dataTableRowSelected':'dataTableRow';?>"
	    onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)">
                <td  width="40%"class="dataTableContent" title="<?php echo $reviews->fields['reviews_text'];?>">
				    <a href="<?php echo zen_href_link(FILENAME_REVIEWS, 
					                                  'page=' . $_GET['page'] . '&rID=' . $reviews->fields['reviews_id'] .'&action=preview');?>">
					  <?php echo zen_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW);?>
					</a>&nbsp;
					<?php if(isset($rInfo) && is_object($rInfo) && ($reviews->fields['reviews_id'] == $rInfo->reviews_id) ){?>
					       <span onclick="document.location.href='<?php echo zen_href_link(FILENAME_REVIEWS,'page='.$_GET['page'].'&rID='.$rInfo->reviews_id .'&action=preview');?>'">
					<?php }else{?>
					       <span onclick="document.location.href='<?php echo zen_href_link(FILENAME_REVIEWS,'page='.$_GET['page'].'&rID='. $reviews->fields['reviews_id']);?>'">
					<?php }?>
					<?php echo zen_get_products_name($reviews->fields['products_id']); ?>
					</span>
				</td>
                <td  width="5%"class="dataTableContent"><?php echo $reviews->fields['customers_name']; ?></td>
                <td class="dataTableContent" align="right"><?php echo zen_image(DIR_WS_TEMPLATE_IMAGES . 'stars_' . $reviews->fields['price_rating'] . '.gif'); ?></td>
				<td class="dataTableContent" align="right"><?php echo zen_image(DIR_WS_TEMPLATE_IMAGES . 'stars_' . $reviews->fields['value_rating'] . '.gif'); ?></td>
				<td class="dataTableContent" align="right"><?php echo zen_image(DIR_WS_TEMPLATE_IMAGES . 'stars_' . $reviews->fields['quality_rating'] . '.gif'); ?></td>
                <td class="dataTableContent" align="right"><?php echo zen_date_short($reviews->fields['date_added']); ?></td>
                <td class="dataTableContent" align="center"> 
		           <span id="span<?php echo $reviews->fields['reviews_id'];?>" 
				         spanid="<?php echo $reviews->fields['reviews_id'];?>" 
						 class="rstatus" status_value="<?php echo $reviews->fields['status'];?>">
					 <?php echo (($reviews->fields['status']=='1')?zen_image(DIR_WS_IMAGES . 'icon_green_on.gif', IMAGE_ICON_STATUS_ON):zen_image(DIR_WS_IMAGES . 'icon_red_on.gif', IMAGE_ICON_STATUS_OFF));?>
				   </span>
                </td>
...
<?php
    ...
    switch ($action) {
      ...
      if (isset($rInfo) && is_object($rInfo)) {
        ...
        //$contents[] = array('text' => TEXT_INFO_REVIEW_RATING . ' ' . zen_image(DIR_WS_TEMPLATE_IMAGES . 'stars_' . $rInfo->price_rating . '.gif'));
		$contents[] = array('text' => 'Price rating' . ' ' . zen_image(DIR_WS_TEMPLATE_IMAGES . 'stars_' . $rInfo->price_rating . '.gif'));
		$contents[] = array('text' => 'Value rating' . ' ' . zen_image(DIR_WS_TEMPLATE_IMAGES . 'stars_' . $rInfo->price_rating . '.gif'));
		$contents[] = array('text' => 'Quality rating' . ' ' . zen_image(DIR_WS_TEMPLATE_IMAGES . 'stars_' . $rInfo->price_rating . '.gif'));
