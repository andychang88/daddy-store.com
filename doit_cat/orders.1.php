  ...
  //#####################Begin:send data to our erp ######################
   //----------------------------------------------------
	...
	function ajax_sendxml($order,$xml_orderid='0')
		...
		if (count($order->products) > 0) {
			...
			foreach ($order->products as $key => $val) {
				...
				$xml_titlename = $val['name'];
				$xml_quantity = $val['qty'];
				$xml_price = $val['price'];
				$xml_final_price = $val['final_price'];
				$xml_model = $val['model'];
				$xml_product.="<item productId='$xml_productId' description='$xml_titlename' quantity='$xml_quantity' price='$xml_price' final_price='$xml_final_price' model='$xml_model'>";
				if (is_array($val['attributes'])) {
					foreach ($val['attributes'] as $key_arr => $val_arr) {
						$xml_attr.="<attribute option='$val_arr[option]' value='$val_arr[value]' prefix='$val_arr[prefix]' price='$val_arr[price]'></attribute>";
					}
				}
	...
	//    $xml_orderid = $oID;//这里是获取的ID
		...
		$xml_currency_type = $order->info['currency'];
		$xml_total = $order->info['total'];
		$xml_shipping_module_code=$order->info['shipping_module_code'];
		$xml_payment_module_code=$order->info['payment_module_code'];
		$xml_customer_id = $order->customer['id'];
		$xml_customer_name = $order->customer['name'];
		$xml_customer_email_address = $order->customer['email_address'];
		$xml_bill_name = $order->billing['name'];
		$xml_bill_street_address = $order->billing['street_address'];
		$xml_bill_suburb = $order->billing['suburb'];
		$xml_bill_city = $order->billing['city'];
		$xml_bill_postcode = $order->billing['postcode'];
		$xml_bill_province = $order->billing['province'];
		$xml_bill_country = $order->billing['country'];
		$xml_deli_name = $order->delivery['name'];
		$xml_deli_street_address = $order->delivery['street_address'];
		$xml_deli_suburb = $order->delivery['suburb'];
		$xml_deli_city = $order->delivery['city'];
		$xml_deli_postcode = $order->delivery['postcode'];
		$xml_deli_state = $order->delivery['state'];
		$xml_deli_country = $order->delivery['country'];
		$xml_discount = "";
		$xml_salesTax = "";
		$xml_info_shippingCost = "";
		foreach ($order->totals as $key => $val) {
			if ($val['class'] == "ot_shipping"){
				$xml_info_shippingCost = str_replace("&euro;", "", $val['text']);
				$xml_info_shippingCost_title = $val['title'];
			}
			if ($val['class'] == "ot_group_pricing") {
				$xml_discount = str_replace("&euro;", "", $val['text']);
				$xml_discount_title = $val['title'];
			}
			if ($val['class'] == "ot_tax"){
				$xml_salesTax = str_replace("&euro;", "", $val['text']);
				$xml_salesTax_title = $val['title'];
			}
		}
	...
	<order id='$xml_orderid' url='$xml_url' siteId='efoxShop'
	signiture='asfdadfadsfadfsaadf' timestamp='$xml_time' currency='$xml_currency_type' total='$xml_total'
		...
		<customer id='$xml_customer_id' title='empty' firstName='$xml_customer_name' lastName='' mail='$xml_customer_email_address'>
			<billAddress>
		...
		<adjustments>
			<adjustment type='discount' description='zhekou1' title='$xml_discount_title' prefix='-' amount='$xml_discount'></adjustment>
	...
	//                echo 'init it ';
					...
					$order = new order($oID);
					$xml_sou = ajax_sendxml($order, $oID);
					error_log($xml_get_orders->fields['orders_id'] . '__', 3, $log_file_name); //send id history 非正常处理，log需要另外生成
					$xml_sendto = urlencode($xml_sou);
	//                    $url = "http://localhost:8012/?xmlfileinfo=" . $xml_sendto;//输出地址
					echo $url = "http://flyinglimy.gicp.net/crmsfa/control/websiteOrderReceiver?order=" . $xml_sendto;
	//                header("location:{$url}");
	//                $c=file_get_contents($url);//改用curl
					$c = cls_curl::get($url);
					var_dump($c);
					echo '--------ok----------';
					exit('send finished');
