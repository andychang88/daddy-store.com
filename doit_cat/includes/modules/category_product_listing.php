...
?>
				...
				/* 
                  if(zen_childs_in_category_count($categories->fields['categories_id'])==0){//最底层的类别设置label Groups
	                  if(zen_check_category_linked_to_label_group($categories->fields['categories_id'])){
	                  	echo '<a href="' . zen_href_link('category_labels.php', 'cateID=' . $categories->fields['categories_id']) . '">' . zen_image(DIR_WS_IMAGES . 'label.gif', 'Edit Label Groups') . '</a>'; 
	                  }else{
	                  	echo '<a href="' . zen_href_link('category_labels.php', 'cateID=' . $categories->fields['categories_id']) . '">' . zen_image(DIR_WS_IMAGES . 'label_disable.gif', 'Edit Label Groups') . '</a>'; 
	                  }
                  } */
                  ?>
...
// find out the rating average from customer reviews
        $reviews = $db->Execute("select (avg(price_rating) / 5 * 100) as average_rating
                                 from " . TABLE_REVIEWS . "
...
<?php if ($action == '') { ?>
                ...
                <td class="dataTableContent" style="width:200px" align="right">
		<?php
		/**
        if(zen_check_product_linked_to_label($products->fields['products_id'])){ 
        	echo '<a href="' . zen_href_link($type_handler, 'cPath=' . $cPath . '&product_type=' . $products->fields['products_type'] . '&pID=' . $products->fields['products_id']  . '&action=modify_labels') . '">' . zen_image(DIR_WS_IMAGES . 'label.gif', 'Modify Product\'s Labels') . '</a>'; 
        }else{
        	echo '<a href="' . zen_href_link($type_handler, 'cPath=' . $cPath . '&product_type=' . $products->fields['products_type'] . '&pID=' . $products->fields['products_id']  . '&action=modify_labels') . '">' . zen_image(DIR_WS_IMAGES . 'label_disable.gif', 'Modify Product\'s Labels') . '</a>'; 
        }**/
        ?>
        <?php
        //推荐产品
		if(mysql_num_rows(mysql_query("select * from  products_recommend where products_id='".$products->fields['products_id']."'"))<1  )
		{
			$k=2;
			
			
			}
		echo '<a href="' . zen_href_link($type_handler, 'cPath=' . $cPath . '&product_type=' . $products->fields['products_type'] . '&pID=' . $products->fields['products_id']  . "&action=tuijian{$k}") . '">'.zen_image(DIR_WS_IMAGES . "tuijian{$k}.gif", 'Recommend').'</a>';
