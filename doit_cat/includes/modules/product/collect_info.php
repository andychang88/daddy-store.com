...
if (!defined('IS_ADMIN_FLAG')) {
  die('Illegal Access');
    ...
    if (isset($_GET['pID']) && empty($_POST)) {
      $product = $db->Execute("select pd.products_name, pd.products_description,pd.products_short_description,pd.products_photo_html,
	                                  pd.products_url,p.products_id, p.products_quantity, p.products_model,p.products_afterbuy_model,									
                                      p.products_image, p.products_price, p.products_virtual, p.products_weight,
                                      p.products_date_added, p.products_last_modified,
                                      date_format(p.products_date_available, '%Y-%m-%d') as
                                      products_date_available, p.products_status,p.products_stock_status,p.products_tax_class_id,
                                      p.manufacturers_id,
    ...
    switch ($pInfo->products_stock_status) {
      case '0': $out_of_stock_status = true; $in_stock_status = false;$being_purchased_status=false;$pre_sale_status=false;break;
      case '1': $out_of_stock_status = false; $in_stock_status = true;$being_purchased_status=false;$pre_sale_status=false;break;
	  case '2': $out_of_stock_status = false; $in_stock_status = false;$being_purchased_status=true;$pre_sale_status=false;break;
	  case '3': $out_of_stokc_status = false; $in_stock_status=false;$being_purchased_status=false;$pre_sale_status=true;break;
    }
...
?>
		  ...
		  <tr>
            <td class="main"><?php echo TEXT_PRODUCTS_STOCK_STATUS; ?></td>
            <td class="main"><?php echo zen_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' 
									  . zen_draw_radio_field('products_stock_status', '0', $out_of_stock_status) . '&nbsp;' . TEXT_PRODUCT_OUT_OF_STOCK . '&nbsp;'
									  . zen_draw_radio_field('products_stock_status', '1', $in_stock_status) . '&nbsp;' . TEXT_PRODUCT_IN_STOCK. '&nbsp;'
									  . zen_draw_radio_field('products_stock_status', '2', $being_purchased_status) .'&nbsp;'.TEXT_PRODUCT_BEING_PURCHASED.'&nbsp;'. zen_draw_radio_field('products_stock_status', '3', $pre_sale_status) .'&nbsp;'.TEXT_PRODUCT_PRE_SALE.'&nbsp;';
							  ?>
		    </td>
          </tr>
          <tr>
...
?>
          <tr class="important_bg">
            <td class="main"><?php if ($i == 0) echo '产品名：'; ?></td>
...
?>
          ...
          <?php 
		       //&&&&&&&&&&&&&&&&&&&&BEGIN:for product short description&&&&&&&&&&&&&&&&&&&&&&&&&&&&
		       //added by john 2010-04-14 
				for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
			?>
					  <tr class="important_bg">
						<td class="main" valign="top"><?php if ($i == 0) echo '产品简短描述：'; ?></td>
						<td colspan="2">
						   <table border="0" cellspacing="0" cellpadding="0">
							 <tr>
								 <td class="main" width="25" valign="top">
									<?php echo zen_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], 
														 $languages[$i]['name']); ?>&nbsp;
								 </td>
						...
						//                $oFCKeditor->Create() ;
								  ...
								  echo zen_draw_textarea_field('products_short_description[' . $languages[$i]['id'] . ']', 'soft', '100%', '30', (isset($products_short_description[$languages[$i]['id']])) ? stripslashes($products_short_description[$languages[$i]['id']]) : zen_get_products_short_description($pInfo->products_id, $languages[$i]['id'])); //,'id="'.'products_description' . $languages[$i]['id'] . '"');
								  } ?>
								 </td>
							 </tr>
						   </table>
...
?>
          <tr  class="important_bg" >
            <td class="main" valign="top"><?php if ($i == 0) echo '产品描述：'; ?></td>
            <td colspan="2">
               <table border="0" cellspacing="0" cellpadding="0">
                 <tr>
                     <td class="main" width="25" valign="top">
...
?>
          ...
          <tr>
            <td class="main"><?php echo '收税类型：'; ?></td>
          ...
          <tr  class="important_bg" >
            <td class="main"><?php echo '产品价格：'; ?></td>
          ...
          <tr>
            <td class="main"><?php echo '指定产品毛重价格：'; ?></td>
          ...
          <tr>
            <td class="main"><?php echo '是否为虚拟产品：'; ?></td>
          ...
          <tr>
            <td class="main" valign="top"><?php echo '是否为免运费产品：'; ?></td>
          ...
          <tr>
            <td class="main"><?php echo '是否显示产品数量框：'; ?></td>
          ...
          <tr>
            <td class="main"><?php echo '一次订单可以购买的最小数量：'; ?></td>
          ...
          <tr>
            <td class="main"><?php echo '一次订单可以购买的最大数量：'; ?></td>
          ...
          <tr>
            <td class="main"><?php echo '产品数量递增单位：'; ?></td>
...
<?php  //added by john 2010-04-14 for product photo html
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
?>
          <tr>
            <td class="main" valign="top"><?php if ($i == 0) echo '产品图片：'; ?></td>
            <td colspan="2">
               <table border="0" cellspacing="0" cellpadding="0">
                 <tr>
                     <td class="main" width="25" valign="top">
                     ...
                     <td class="main" width="100%">
					   <?php if ($_SESSION['html_editor_preference_status']=="FCKEDITOR") {
                            $oFCKeditor = new FCKeditor('products_photo_html[' . $languages[$i]['id'] . ']') ;
                            $oFCKeditor->Value = (isset($products_photo_html[$languages[$i]['id']])) ? stripslashes($products_photo_html[$languages[$i]['id']]) : zen_get_products_photo_html($pInfo->products_id, $languages[$i]['id']) ;
                            $oFCKeditor->Width  = '99%' ;
                            $oFCKeditor->Height = '350' ;
                            $output = $oFCKeditor->CreateHtml() ;  echo $output;
                      } else { // using HTMLAREA or just raw "source"
            
                      echo zen_draw_textarea_field('products_photo_html[' . $languages[$i]['id'] . ']', 'soft', '100%', '30', (isset($products_photo_html[$languages[$i]['id']])) ? stripslashes($products_photo_html[$languages[$i]['id']]) : zen_get_products_photo_html($pInfo->products_id, $languages[$i]['id']));
                      } ?>
                     </td>
                 </tr>
               </table>
...
?>
          ...
          <tr  class="important_bg" >
            <td class="main"><?php echo '产品数量：'; ?></td>
          ...
          <tr  class="important_bg" >
            <td class="main"><?php echo '产品型号：'; ?></td>
		  ...
		  <tr  class="important_bg" >
		     <td class="main"><?php echo '产品afterbuy型号：'; ?></td>
             <td class="main"><?php echo zen_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . zen_draw_input_field('products_afterbuy_model', $pInfo->products_afterbuy_model, zen_set_field_length(TABLE_PRODUCTS, 'products_afterbuy_model')); ?></td>
		  </tr>
          <tr>
            <td colspan="2"><?php echo zen_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
...
?>
          ...
          <tr   class="important_bg" >
            <td class="main" colspan="2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="main"><?php echo '产品图片：'; ?></td>
...
?>
          ...
          <tr  class="important_bg" >
            <td class="main"><?php echo '产品重量（单位g）：'; ?></td>
          ...
          <tr>
            <td class="main"><?php echo '产品排序：'; ?></td>
      ...
      <tr>
        <td class="main" align="right"><?php echo zen_draw_hidden_field('products_date_added', (zen_not_null($pInfo->products_date_added) ? $pInfo->products_date_added : date('Y-m-d'))) . zen_image_submit('button_preview.gif', '预览') . '&nbsp;&nbsp;<a href="' . zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . (isset($_GET['pID']) ? '&pID=' . $_GET['pID'] : '') . (isset($_GET['page']) ? '&page=' . $_GET['page'] : '')) . '">' . zen_image_button('button_cancel.gif', '取消') . '</a>'; ?></td>
      </tr>
    </table></form>
