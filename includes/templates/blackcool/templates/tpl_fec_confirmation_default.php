...
<?php 
  ...
  echo '<div id="onePageText">' . FEC_CHECKOUT_CONFIRMATION_TEXT2;
?>
<?php
...
?>
  ...
  <?php
    ...
    if (is_array($payment_modules->modules)) {
	  ...
	  //begin--------
		if(trim($GLOBALS['payment_modules']->selected_module)=='paydollar')//paydollar  20100917 14:45
		  {
				$m_change_currency=array();
				$m_change_currency=array('m_type'=>$_SESSION['currency'],
										 'm_info'=>$GLOBALS['currencies']->currencies
										 );               
				global $m_change_currency;
				echo $payment_modules->process_button();
		  }
		  else  //default
		  {
...
// create the order record  防漏单 
 /**
  * 防漏单模块做好之后，后台的漏单为GS/AK，GS/AK的为防漏单的订单，
  * 只有在出现漏单的时候才会查看这些订单，里面的信息跟正常订单相比会少一个交易号，
  * 但是至少是可以看到客户信息及所购买产品的
  */
/**
