...
//get specials price or sale price
  function zen_get_products_special_price($product_id, $specials_price_only=false) {
    global $db;
    $product = $db->Execute("select products_price, products_model, products_priced_by_attribute 
							 from   " . TABLE_PRODUCTS . " 
...
// show case only
    ...
    if ($display_special_price) {
      $show_normal_price = '<span class="normalprice">' .TEXT_PRICE. $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . ' </span>';
      if ($display_sale_price && $display_sale_price != $display_special_price) {
        ...
        if ($product_check->fields['product_is_free'] == '1') {
          ...
          $show_special_price = '&nbsp;' . '<br /><span class="productSpecialPrice">'.TEXT_SPECIAL_PRICE . $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span>';
        }
        $show_sale_price = '';
      }
    } else {
      if ($display_sale_price) {
        ...
        if ($product_check->fields['product_is_free'] == '1') {
          $show_normal_price = '<s>'.TEXT_PRICE . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</s>';
        } else {
          
if(106860 != $_GET['products_id'])
{
    $show_normal_price = '<span class="normalprice" id="normalprice" >' .TEXT_PRICE. $currencies->display_price($display_normal_price*1.43, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . ' </span>';
}
		  
		  //20101203 add it
         // $show_normal_price .= TEXT_SPECIAL_PRICE.$currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])); 
		  $show_normal_price .= '&nbsp;' . '<br /><span class="productSpecialPrice" >'.TEXT_SPECIAL_PRICE . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . ' </span>';
		  
            //$show_normal_price = TEXT_PRICE.$currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id']));//source 
        }
        $show_special_price = '';
        $show_sale_price = '';
      }
