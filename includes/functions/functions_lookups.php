...
//    $new_range = ' and date_format(p.products_date_available, \'%Y%m%d\') >' . $zc_new_date;
  ...
  function getNewProductsByCatId($catId, $limit = 5, $products_id_str = '') {
	 ...
	 $new_result = $db->Execute($new_sql);
	 
	 if( $new_result -> RecordCount() > 0 ){
	 	while( !$new_result -> EOF ){
	 		
	 		$tmp_arr = array();
	 		$tmp_products_id = $new_result -> fields['products_id'];
	 		
	 		$tmp_arr['products_name'] = zen_get_products_name( $tmp_products_id );
	 		$tmp_arr['products_image'] = $new_result -> fields['products_image'];
	 		$tmp_arr['products_price'] = $new_result -> fields['products_price'];
	 		$tmp_arr['buy_link'] = zen_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $tmp_products_id);
	 		$tmp_arr['products_url_link'] = zen_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $tmp_products_id);
...
function my_microtime_float($dec = 3) {
