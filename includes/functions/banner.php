...
function zen_display_banner_by_groupd($group_name){
	global $db;
	$banner_sql='select banners_id, banners_title, banners_image, 
                       banners_html_text, banners_open_new_windows, banners_url
                from ' . TABLE_BANNERS . '
                where status = 1
                and  banners_group="'.$group_name.'"  order by banners_sort_order asc,banners_id asc ';
   $banner=$db->Execute($banner_sql);
   
   $banner_string = '';
   
   if($banner->RecordCount()>0){
     while(!$banner->EOF){
     	$banner_string .= '<a href="' . zen_href_link(FILENAME_REDIRECT, 'action=banner&goto=' . $banner->fields['banners_id']) . 
     	'" '.	($banner->fields['banners_open_new_windows'] == '1'?'target="_blank"':'')	.	
     	'>' . zen_image(DIR_WS_IMAGES . $banner->fields['banners_image'], $banner->fields['banners_title']) . 
     	'</a>';
