  ...
  function count_contents() {
    ...
    if (is_array($this->contents)) {
      ...
      while (list($products_id, ) = each($this->contents)) {
		...
		if($total_items>400000)$total_items=400000;
      }
    }
  ...
  function actionUpdateProduct($goto, $parameters) {
    ...
    for ($i=0, $n=sizeof($_POST['products_id']); $i<$n; $i++) {
      ...
      if ( in_array($_POST['products_id'][$i], (is_array($_POST['cart_delete']) ? $_POST['cart_delete'] : array())) or $_POST['cart_quantity'][$i]==0) {
		...
		if($new_qty>400000)$new_qty=400000;
...
// display message if all is good and not on shopping_cart page
  ...
  function actionAJAXUpdateProduct($pid,$p_qty,$p_attributes,$p_delete=false) {
    global $messageStack;
	
	//if(!isset($pid) || !is_numeric($pid) || $pid<=0) return;
	if(!isset($p_qty) || !is_numeric($p_qty) || $p_qty<0) return;
	
	if($p_delete || $p_qty==0){
		$this->remove(zen_get_uprid($pid,$p_attributes));
	}else{
		$add_max = zen_get_products_quantity_order_max($pid);
        $cart_qty = $this->in_cart_mixed($pid);
        $new_qty = $p_qty;
		
		$new_qty = $this->adjust_quantity($new_qty, $pid, 'shopping_cart');
		 ...
		 if (($add_max == 1 and $cart_qty == 1)) {
			  ...
			  $this->add_cart($pid, $new_qty,$p_attributes, false);
		 }
		 if ($adjust_max == 'true') {
		 	  $messageStack->add_session('shopping_cart', ERROR_MAXIMUM_QTY.zen_get_products_name($pid),'caution');
		 }
	}
  }
  /**
