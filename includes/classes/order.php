 ...
 * @version $Id: order.php 100 2010-01-19 19:33:07Z numinix $
...
*/
    ...
    for ($i=0, $n=sizeof($products); $i<$n; $i++) {
      ...
      $this->products[$index] = array('qty' => $products[$i]['quantity'],
                                      ...
                                      'final_price' => $products[$i]['price'] + $_SESSION['cart']->attributes_price(zen_get_uprid($products[$i]['id'], $products[$i]['attributes'])),
   ...
   if (DISPLAY_PRICE_WITH_TAX == 'true') {
		  if($_SESSION['insurance']!=""){//判断保险是否选中 2011-3-29 du
...
*/
  ...
  function create($zf_ot_modules, $zf_mode = 2) {
    ...
    $sql_data_array = array('customers_id' => $_SESSION['customer_id'],
							...
							'insurance' => floatval($_SESSION['insurance']),//此处是增加的字段，用于保险的，但后台我并未输出这字段，我把保险放进了总价里，在总价的标题便可看出是否购买了保险  2011.3.29 du
...
if(isset($_SESSION['old_orders_id']) && !empty($_SESSION['old_orders_id'])){
	$old_orders_id=$_SESSION['old_orders_id'];
	$where = " orders_id='".$old_orders_id."'";
	$action = "update";
	$delete_old_prod_sql = "delete from ".TABLE_ORDERS_PRODUCTS." where ".$where;
	$db->Execute($delete_old_prod_sql);
	$delete_old_attr_sql = "delete from ".TABLE_ORDERS_PRODUCTS_ATTRIBUTES." where ".$where;
	$db->Execute($delete_old_attr_sql);
	unset($_SESSION['old_orders_id']);
}else{
	$where = "";
	$action = "insert";
}
...
// COWOA Conditional
    ...
    if (SEND_EXTRA_ORDER_EMAILS_TO != '') {
      ...
      $email_order . $extra_info['TEXT'], STORE_NAME, EMAIL_FROM, $html_msg,$email_module_template, $this->attachArray);
