...
class paydollar extends base {
  ...
  function selection() {
   return array('id' => $this->code,
                'module' => $this->title,
				'icon'=>'<img src="images/paydollar.gif"/>');
  }
  /**
 ...
 function process_button() {
        global $_SERVER, $order,$m_change_currency;

        $this->totalsum = $order->info['total'];
// change begin   20100917 14:45
        $usd_value=1;//set default
        $customer_value=1;//set default
        $usd_value=$m_change_currency["m_info"]["USD"]["value"];
        foreach($m_change_currency["m_info"] as $loop_key=>$loop_value)
        {
            if($m_change_currency["m_type"]==$loop_key)
            {
                $customer_value=$loop_value['value'];
                break;
            }
        }
        //$this->totalsum =round($this->totalsum/$customer_value*$usd_value,2);
		$this->totalsum=round($this->totalsum*$usd_value,2);
//end change
