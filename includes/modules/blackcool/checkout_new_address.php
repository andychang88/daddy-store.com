...
if (isset($_POST['fecaction']) && ($_POST['fecaction'] == 'submit') || isset($_POST['action'])) {
  // process a new address
  if (zen_not_null($_POST['firstname']) && zen_not_null($_POST['lastname']) && zen_not_null($_POST['street_address'])  && ($_POST['action'] == 'new_address')) {
    $process = true;
...
//echo ' I SEE: country=' . $country . '&nbsp;&nbsp;&nbsp;state=' . $state . '&nbsp;&nbsp;&nbsp;zone_id=' . $zone_id;
    if (ACCOUNT_GENDER == 'true') {
      if ( ($gender != 'm') && ($gender != 'f') ) {
        $error = true;
        $messageStack->add('checkout_new_address', ENTRY_GENDER_ERROR);
      }
    }

    if (strlen($firstname) < ENTRY_FIRST_NAME_MIN_LENGTH) {
      $error = true;
      $messageStack->add('checkout_new_address', ENTRY_FIRST_NAME_ERROR);
    }

    if (strlen($lastname) < ENTRY_LAST_NAME_MIN_LENGTH) {
      $error = true;
      $messageStack->add('checkout_new_address', ENTRY_LAST_NAME_ERROR);
    }

    if (strlen($street_address) < ENTRY_STREET_ADDRESS_MIN_LENGTH) {
      $error = true;
      $messageStack->add('checkout_new_address', ENTRY_STREET_ADDRESS_ERROR);
    }

    if (strlen($city) < ENTRY_CITY_MIN_LENGTH) {
      $error = true;
      $messageStack->add('checkout_new_address', ENTRY_CITY_ERROR);
    }
    ...
    if (ACCOUNT_STATE == 'true' && zen_not_null($_POST['state'])) {
      ...
      if ($entry_state_has_zones == true) {
		 echo $zone_query = "SELECT distinct zone_id, zone_name, zone_code
						   FROM " . TABLE_ZONES . "
						   WHERE zone_country_id = :zoneCountryID
						   AND " . 
		    ...
		    if ($found_exact_iso_match) {
				...
				$messageStack->add('checkout_new_address', ENTRY_STATE_ERROR_SELECT);
		    }
    } else {
		  if (strlen($state) < ENTRY_STATE_MIN_LENGTH) {
			$error = true;
			$error_state_input = true;
			$messageStack->add('checkout_new_address', ENTRY_STATE_ERROR);
		  }
      }
    }
...
/*    if (strlen($postcode) < ENTRY_POSTCODE_MIN_LENGTH) {
      $error = true;
      $messageStack->add('checkout_new_address', ENTRY_POST_CODE_ERROR);
    }*/

    if ( (is_numeric($country) == false) || ($country < 1) ) {
      $error = true;
      $messageStack->add('checkout_new_address', ENTRY_COUNTRY_ERROR);
    }
  ...
  } elseif (isset($_POST['address'])  && ($_POST['action'] == 'choose_address')) {
    switch($addressType) {
      case 'billto':
      $reset_payment = false;
      if ($_SESSION['billto']) {
