...
$sql = "select count(*) as total
          ...
          where    patrib.products_id='" . (int)$_GET['products_id'] . "'
            ...
            if ($pr_attr->fields['total'] > 0) {
              ...
              //################Start:Make sure the customer must select one value###############1/2
				$check_must_select=array();
			  //################End:Make sure the customer must select one value################1/2
              ...
              and       pov.language_id = '" . (int)$_SESSION['languages_id'] . "' " .
                ...
                while (!$products_options->EOF) {
				  //################Start:Make sure the customer must select one value###############2/2
				    if($products_options->fields['attributes_display_only']==1 
					   && !(isset($check_must_select[$products_options_names->fields['products_options_id']])) ){
					  $check_must_select[$products_options_names->fields['products_options_id']]=$products_options->fields['options_values_id'];
					}
				  //################End:Make sure the customer must select one value################2/2
                  // reset
