...
switch ($_GET['main_page']) {
  ...
  define('TOP_HEAD_H1_TITLE',TOP_HEAD_H1_TITLE_HOME_PAGE);
  break;

  case 'index':
  // bof: categories meta tags
  // run custom categories meta tags
  $sql = "select * from " . TABLE_METATAGS_CATEGORIES_DESCRIPTION . " mcd 
  ...
  $sql2 = " select cd.categories_name 
			from " . TABLE_CATEGORIES . ' c, 
				 ' . TABLE_CATEGORIES_DESCRIPTION . " cd 
			...
			and   cd.language_id = '" . (int)$_SESSION['languages_id'] . "' 
			and   c.categories_status=1";
 $category_metatags2 = $db->Execute($sql2);
  
  if (!$category_metatags->EOF) {
		//####################BEGIN:better experience for seo #############
		
		//####################END:better experience for seo ###############
		/*define('META_TAG_TITLE', str_replace('"','',$category_metatags->fields['metatags_title']));
		define('META_TAG_DESCRIPTION', str_replace('"','',$category_metatags->fields['metatags_title'].' ,'.$category_metatags->fields['metatags_description']));
		define('META_TAG_KEYWORDS', str_replace('"','',$category_metatags->fields['metatags_keywords']));
		define('TOP_HEAD_H1_TITLE',str_replace('"','',zen_clean_html($category_metatags2->fields['categories_name'])));*/
		define('META_TAG_TITLE', str_replace('"','',$category_metatags->fields['metatags_title'].'-'.HOME_PAGE_TITLE));
		define('META_TAG_DESCRIPTION',str_replace('"','',$category_metatags2->fields['categories_name'].'-'.$category_metatags->fields['metatags_description']));
		define('META_TAG_KEYWORDS',str_replace('"','',$category_metatags2->fields['categories_name']));
    ...
    if ($category_depth == 'nested') {
      ...
      if ($category_metatags->EOF) {	    
	    ...
	    //Modified by john 2010-06-02 FOR SEO MODIFICATION 1/
		...
		define('META_TAG_TITLE', str_replace('"','', zen_clean_html($category_metatags->fields['categories_name']).'-'.HOME_PAGE_TITLE));
        define('META_TAG_DESCRIPTION', str_replace('"','', zen_clean_html($category_metatags->fields['categories_name']) .'-'.HOME_PAGE_META_DESCRIPTION));
        define('META_TAG_KEYWORDS', str_replace('"','',zen_clean_html($category_metatags->fields['categories_name'])));
      } // EOF
      ...
      if (isset($_GET['manufacturers_id']) || ((isset($_GET['filter_id']) && $_GET['filter_id'] > 0) && isset($_GET['cPath'])) ) {
        ...
        if ($category_metatags2->EOF) {
          ...
          define('META_TAG_DESCRIPTION', str_replace('"','',TITLE . PRIMARY_SECTION . zen_clean_html($category_metatags->fields['categories_name']) . SECONDARY_SECTION . KEYWORDS));
          define('META_TAG_KEYWORDS', str_replace('"','',KEYWORDS . METATAGS_DIVIDER . zen_clean_html($category_metatags->fields['categories_name'])));*/
		  define('META_TAG_TITLE', str_replace('"','', zen_clean_html($category_metatags2->fields['categories_name']).'-'.HOME_PAGE_TITLE));
          define('META_TAG_DESCRIPTION', str_replace('"','', zen_clean_html($category_metatags2->fields['categories_name']) .'-'.HOME_PAGE_META_DESCRIPTION));
          define('META_TAG_KEYWORDS', str_replace('"','',zen_clean_html($category_metatags2->fields['categories_name'])));		 
        } // EOF
      }
    } else {
  ...
  */
  ...
  if ($product_info_metatags->EOF) {
	...
	/* $sql_n="select  cd.categories_name					  
			 from ".TABLE_PRODUCTS_TO_CATEGORIES." p2c,
			      ".TABLE_PRODUCTS." p,
				  ".TABLE_CATEGORIES_DESCRIPTION." cd
			 where p2c.products_id=".(int)$_GET['products_id']." 
			 and   p2c.products_id=p.products_id 
			 and   p2c.categories_id=p.master_categories_id 
			 and   p2c.categories_id=cd.categories_id 
			 and   cd.language_id=".(int)$_SESSION['languages_id'];
	  $product_cate=$db->Execute($sql_n);
	  $cate_name=$product_cate->fields['categories_name'];*/
	  
      $meta_products_name = '';
	  $meta_products_name =$product_info_metatags->fields['products_name'].'-'.HOME_PAGE_TITLE;
      //$meta_products_price = '';
      $metatags_keywords = '';
	  ...
	  //Modified by john 2010-06-02 FOR SEO MODIFICATION 2/
	  ...
	  $sql= "select   pd.products_name, 
			 ...
			 and   pd.language_id=".(int)$_SESSION['languages_id'];
	  $product_info_metatags=$db->Execute($sql);
	  $meta_product_name=zen_clean_html($product_info_metatags->fields['products_name']); 
	  $meta_category_name=zen_clean_html($product_info_metatags->fields['categories_name']);
	  
      define('META_TAG_TITLE', str_replace('"','',$meta_product_name.'-'.$meta_category_name.'-'.HOME_PAGE_TITLE));
      define('META_TAG_DESCRIPTION', str_replace('"','',$meta_product_name.'-'.$meta_category_name.'-'.HOME_PAGE_META_DESCRIPTION));
