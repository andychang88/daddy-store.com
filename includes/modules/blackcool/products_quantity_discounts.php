...
$products_min_query = $db->Execute("select products_quantity_order_min 
                                    from " . TABLE_PRODUCTS . " 
...
$products_discounts_query = $db->Execute("select * 
...
$quantityDiscountsJS=array();
$columnCount = 0;

if ($products_discounts_query->EOF) {
	$quantityDiscountsJS[]=array('qty'=>1,
                               'price'=>$product_info->fields['products_price']
							   );
}
...
while (!$products_discounts_query->EOF) {
  ...
  /***********generate discount array for JS control*********/
  $quantityDiscountsJS[]=array('qty'=>$products_discounts_query->fields['discount_qty'],
                               'price'=>$currencies->value($quantityDiscounts[$columnCount]['discounted_price'],true)
							   );
  /**********************************************************/ 
