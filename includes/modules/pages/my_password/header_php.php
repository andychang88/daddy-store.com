...
if (isset($_SESSION['update_token']) && $_SESSION['update_token']==$_POST['update_token'] && isset($_POST['action']) && ($_POST['action'] == 'process')) {
  ...
  if (strlen($password_current) < ENTRY_PASSWORD_MIN_LENGTH) {
    ...
    $messageStack->add('my_password', ENTRY_PASSWORD_CURRENT_ERROR);
  } elseif (strlen($password_new) < ENTRY_PASSWORD_MIN_LENGTH) {
    $error = true;

    $messageStack->add('my_password', ENTRY_PASSWORD_NEW_ERROR);
  } elseif ($password_new != $password_confirmation) {
    $error = true;

    $messageStack->add('my_password', ENTRY_PASSWORD_NEW_ERROR_NOT_MATCHING);
  }
  ...
  if ($error == false) {
    ...
    if (zen_validate_password($password_current, $check_customer->fields['customers_password'])) {
     ...
     // zen_redirect(zen_href_link('my_password', 'update=success', 'SSL'));
    } else {
      $error = true;
      $messageStack->add('my_password', ERROR_CURRENT_PASSWORD_NOT_MATCHING);
    }
  }
}
$_SESSION['update_token']=time();
