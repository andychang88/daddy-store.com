...
//added by john 2010-04-03
$order_payment_method=$orders->fields['payment_method'];
$order_shipping_method=$orders->fields['shipping_method'];
$order_paid_total=$currencies->format($orders->fields['order_total'], true, $orders->fields['currency'], $orders->fields['currency_value']);
//end
...
if ($flag_global_notifications != '1') {
  ...
  $products_array = array();//store for google order transaction track
  $counter = 0;

  $products_query = "SELECT products_id, products_name,products_model,products_quantity,products_price
                     FROM " . TABLE_ORDERS_PRODUCTS . "
                     WHERE orders_id = :ordersID
                     ORDER BY products_name";
  ...
  while (!$products->EOF) {
		...
		if(zen_not_null($products->fields['products_model'])){
			$product_sku=$products->fields['products_model'];
		}else{
			$product_sku=$products->fields['products_id'];
		}
		$products_array[]=array('order_id'=>$orders_id,
								'product_name'=>substr($products->fields['products_name'],0,30).'...',
								'product_sku'=>$product_sku,
								'product_qty'=>$products->fields['products_quantity'],
								'product_price'=>$products->fields['products_price']
								);
		$products->MoveNext();
  }
