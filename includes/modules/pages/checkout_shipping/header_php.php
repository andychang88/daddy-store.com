...
// get all available shipping quotes
  $quotes = $shipping_modules->quote();
   ...
   //安装指定的顺序排序bof
  $tmp_quotes = array();
  foreach ($quotes as $tmp_quote){
  	$quote_id = $tmp_quote['id'];
  	$sort_str = 'MODULE_SHIPPING_'.strtoupper($quote_id).'_SORT_ORDER';

  	if( defined($sort_str) && is_numeric(constant($sort_str)) ) {
  		$tmp_quote['sort'] = constant($sort_str);
  	}else{
  		$tmp_quote['sort'] = 100;
  	}
  	
  	$tmp_quotes[]=$tmp_quote;
  }
  if(count($tmp_quotes)>0){
  	usort($tmp_quotes, "cmp_shipping_method");
    $quotes = $tmp_quotes;
  }
  
  
  function cmp_shipping_method($a, $b){
  	if ($a['sort'] == $b['sort']) {
        return 0;
    }
