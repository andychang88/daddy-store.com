...
$_GET['keyword'] = urldecode(trim($_GET['keyword']));
...
/*$select_str = "SELECT DISTINCT " . $select_column_list .
              " m.manufacturers_id, p.products_id, pd.products_name, p.products_price, p.products_tax_class_id, 
			    pd.products_short_description,p.products_price_sorter, p.products_qty_box_status, p.master_categories_id ";*/
$select_str = "SELECT DISTINCT " . $select_column_list .
...
$from_str.=TABLE_PRODUCTS_DESCRIPTION . " pd)
		   ...
		   ON mtpd.products_id= pd.products_id
...
if (isset($keywords) && zen_not_null($keywords)) {
	...
	/*end 2011-06-08 zhengcongzhen 屏蔽搜索 windows 字眼*/
  if (zen_parse_search_string(stripslashes($_GET['keyword']), $search_keywords)) {
    ...
    for ($i=0, $n=sizeof($search_keywords); $i<$n; $i++ ) {
      switch ($search_keywords[$i]) {
        ...
        break;
        default:
		/*加入兼容型号的搜索--Modified by john 2010-05-14*/
		/*$where_str .= "(pd.products_name LIKE '%:keywords%'
                                         OR p.products_model
                                         LIKE '%:keywords%'
                                         OR m.manufacturers_name
                                         LIKE '%:keywords%'";*/
        //no need to search manufacturer info 
        $where_str .= "(pd.products_name LIKE '%:keywords%'
                        OR p.products_model LIKE '%:keywords%' OR p.products_afterbuy_model LIKE '%:keywords%'
						";

        $where_str = $db->bindVars($where_str, ':keywords', $search_keywords[$i], 'noquotestring');
        // search meta tags
        $where_str .= " OR (MATCH(mtpd.metatags_keywords)AGAINST(':keywords') 
                        AND mtpd.metatags_keywords !='')";

        $where_str = $db->bindVars($where_str, ':keywords', $search_keywords[$i], 'noquotestring');

        $where_str .= " OR ( MATCH(mtpd.metatags_description)AGAINST(':keywords') 
                        AND mtpd.metatags_description !='')";

        $where_str = $db->bindVars($where_str, ':keywords', $search_keywords[$i], 'noquotestring');

        if (isset($_GET['search_in_description']) && ($_GET['search_in_description'] == '1')) {
          $where_str .= " OR (MATCH(pd.products_description)AGAINST(':keywords')) ";
		  $where_str .= " OR (MATCH(pd.products_short_description)AGAINST(':keywords')) ";
...
//####Begin:save the keyword or not,up to search result ###########1/2
//added by john 2010-08-21
$has_search_result_this_keyword=true;
//####End:save the keyword or not,up to search result #############
$result = new splitPageResults($listing_sql, MAX_DISPLAY_PRODUCTS_LISTING, 'p.products_id', 'page');
if ($result->number_of_rows == 0) {
  //####Begin:save the keyword or not,up to search result ###########2/2
  //added by john 2010-08-21
  $has_search_result_this_keyword=false;
  //####End:save the keyword or not,up to search result #############
