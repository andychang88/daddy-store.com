...
//######################BOF:show paypal express checkout button in shopping cart page###############
//added by john 2010-06-24
...
$ec_button_enabled = ($paypalec_enabled && ($flagHasCartContents && $_SESSION['cart']->total > 0));
//added by changanti begin user
if(isset($_GET['redirect_to_shipping']) && isset($_GET['old_orders_id'])){
	zen_redirect(zen_href_link('checkout_shipping', '', 'SSL'));
}else{
	unset($_SESSION['old_orders_id'],$_SESSION['old_payment'],$_SESSION['old_shipping']);
}
$flow_shopping_cart=true;

if(!empty($_GET['get_cart_p']) && $_GET['get_cart_p']==1){
  ob_clean();
  die(json_encode(array('status'=>'1','html'=>$productArray)));
  
  
}
//added by changanti end user
//######################BOF:show paypal express checkout button in shopping cart page###############
