...
define('ACCOUNT_STATE_MY',false);//禁用state
/**
 * Process deletes
 */
if (isset($_GET['action']) &&  isset($_GET['delete']) && is_numeric($_GET['delete'])) {
  ...
  $messageStack->add_session('my_address_book', SUCCESS_ADDRESS_BOOK_ENTRY_DELETED, 'success');

  zen_redirect(zen_href_link('my_address_book', 'ucenter=1', 'SSL'));
}
...
if (isset($_POST['action']) && (($_POST['action'] == 'process') || ($_POST['action'] == 'update'))) {
  ...
  $telphone = zen_db_prepare_input($_POST['telphone']);
  ...
  if ($error == false) {
    $sql_data_array= array(array('fieldName'=>'entry_firstname', 'value'=>$firstname, 'type'=>'string'),
                           ...
                           array('fieldName'=>'telphone', 'value'=>$telphone, 'type'=>'string'),
...
if (isset($_GET['edit']) && is_numeric($_GET['edit'])) {
  $entry_query = "SELECT entry_gender, entry_company, entry_firstname, entry_lastname,
                         ...
                         entry_state, entry_zone_id, entry_country_id, email,telphone 
