...
// +----------------------------------------------------------------------+
//  $Id: header_php.php 107 2010-03-14 20:57:10Z numinix $
//
...
//added by changanti begin user
  $flow_checkout=true;
//added by changanti add  user
  //
  ...
  if (!$_SESSION['customer_id']) {
	...
	$_SESSION['login_status'] = 0;//设置未登陆状态 zhengcongzhen
  ...
  G('initShippingModuleBegin');
  $shipping_modules = new shipping();
  G('initShippingModuleEnd');
//echo  "<br>".__FILE__.' line:'.__LINE__.'============'."<pre>";print_r(G());exit;
  G('initSessioinSendToBillToBegin');
  ...
  G('initSessioinSendToBillToEnd');
  
  //####################further step for easy signup####################################
  // to finish the address-book,customer info such as name,default address id
  //added by john 2010-07-15
  if((!$_SESSION['sendto']) || ($_SESSION['customer_default_address_id']==0) || empty($_SESSION['customer_first_name']) || empty($_SESSION['customer_last_name'])){
   include(DIR_WS_MODULES . zen_get_module_directory(FILENAME_EASY_CREATE_ACCOUNT_FURTHER_FC));
  }
  //####################END:further step for easy signup################################

  // get all available shipping quotes

 	$quotes = $shipping_modules->quote();
//echo  "<br>".__FILE__.' line:'.__LINE__.'============'."<pre>";print_r($quotes);exit;
   //print_r($quotes);
//added by changanti begin
//如果用户在用户中心的未支付的订单中点击支付，需要恢复用户原来的选择
   if(isset($_SESSION['old_orders_id']) && isset($_SESSION['payment']) && isset($_SESSION['shipping']) ){
 	$shipping_id=$_SESSION['shipping'];
 	
 	foreach ($quotes as $q){
 		if($q['id']==$shipping_id){
 			$_SESSION['shipping']=$q['methods'][0];
 			$_SESSION['shipping']['id']=$q['id'].'_'.$shipping_id;
 			break;
 		}
 	}
 	if(isset($_SESSION['old_coupon']) && !empty($_SESSION['old_coupon'])){
 		$tmp_result=$db->Execute("select coupon_id from ".TABLE_COUPONS." where coupon_code='".$_SESSION['old_coupon']."'");
 		if($tmp_result->RecordCount()>0){
 			$_SESSION['cc_id']=$tmp_result->fields['coupon_id'];
 		}
 	}
  }
//added by changanti end
  ...
  if ( !$_SESSION['shipping'] || ( $_SESSION['shipping'] && ($_SESSION['shipping'] == false) && (zen_count_shipping_modules() > 1) ) ) {
  	...
  	G('StatisCheapestShippingBegin');
    $_SESSION['shipping'] = $shipping_modules->cheapest();
    G('StatisCheapestShippingEnd');
...
//echo "<pre>";print_r(G());exit;
  ...
  case 'update':
	...
	if($_POST['shipping']==$_SESSION["shipping"]["id"] && $_POST['dc_redeem_code']==""){
		if ($_SESSION['insurance']!=$_POST['insurance']) {
		  $_SESSION['insurance'] = floatval($_POST['insurance']);
		}
	}
    ...
    if ( (zen_count_shipping_modules() > 0) || ($free_shipping == true) ) {
      if ( (isset($_POST['shipping'])) && (strpos($_POST['shipping'], '_')) ) {
        ...
        if ( is_object($$module) || ($_SESSION['shipping'] == 'free_free') ) {
          ...
          }else {$GLOBALS['testest'] = 'test';
            // avoid incorrect calculations during redirect
            $shipping_modules = new shipping();//echo  "<br>".__FILE__.' line:'.__LINE__.'============'."<pre>";echo '---session:';echo zen_count_shipping_modules();echo '---'.$_GET['fecaction'];exit;
  ...
  case 'submit':
    ...
    break;
  case 'null':
    $form_action_url = zen_href_link(FILENAME_FEC_CONFIRMATION, 'action=process', 'SSL');
    break;
  default:
