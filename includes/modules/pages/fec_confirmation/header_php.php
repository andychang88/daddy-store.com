 ...
 * @version $Id: header_php.php 106 2010-03-14 20:55:15Z numinix $
 */
...
// if the customer is not logged on, redirect them to the login page
  if (!$_SESSION['customer_id']) {
    $_SESSION['navigation']->set_snapshot(array('mode' => 'SSL', 'page' => FILENAME_CHECKOUT));
      //zen_redirect(zen_href_link(FILENAME_LOGIN, '', 'SSL'));//zhengcongzhen  未登陆时刷当前页（使用弹出式注册登陆）
      zen_redirect(zen_href_link('checkout', '', 'SSL'));
  } else {
    // validate customer
...
if (!$_SESSION['shipping'] || !$_SESSION['sendto']) {
