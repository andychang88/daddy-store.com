...
// class constructor
    function ems() {
	  ...
	  if(!defined('MODULE_SHIPPING_EMS_DISABLED_COUNTRIES'))  $this->enabled = false;
...
// class methods
    function quote($method = '') {
      ...
      if(defined('MODULE_SHIPPING_EMS_BASIS_WEIGHT_FEE') && 
	     defined('MODULE_SHIPPING_EMS_BASIS_WEIGHT') &&
	     defined('MODULE_SHIPPING_EMS_REST_WEIGHT_UNIT_FEE') && 
		 MODULE_SHIPPING_EMS_REST_WEIGHT_UNIT_FEE>0){
      ...
      if ($error == true) $this->quotes['error'] = MODULE_SHIPPING_EMS_NOT_DISPATCH_TO_TEXT.' '.$dest_country;
    ...
    function install() {
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description,
					...
					VALUES ('Enable EMS shipping Method', 'MODULE_SHIPPING_EMS_STATUS', 'True', 
	  ...
	  $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, 
				    ...
				    values ('Setting for the handling fee for this shipping', 'MODULE_SHIPPING_EMS_HANDLING_FEE', '', 
					        'handling fee for per package:', '6', '0',now())");      
							
	  $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, 
	                                                         configuration_group_id, sort_order,date_added) 
					values ('Set the Basis-Weight for ems','MODULE_SHIPPING_EMS_BASIS_WEIGHT', '',
					        'you can set the basis weight here and the weigh-unit is gram', '6', '0', now())");  
							
	  $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, 
	                                                         configuration_group_id, sort_order,date_added) 
					values ('Set the Basis-Weight-Fee for ems ','MODULE_SHIPPING_EMS_BASIS_WEIGHT_FEE', '',
					        'you can set the basis-weight-fee here', '6', '0', now())");   
							
	  $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, 
	                                                         configuration_group_id, sort_order,date_added) 
					values ('Set the Exclude-Basis-Weight Unit for ems','MODULE_SHIPPING_EMS_REST_WEIGHT_UNIT', '',
					        'you can set the exclude basis weight unit here and the weigh-unit is gram ', '6', '0', now())");  
							
	  $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, 
	                                                         configuration_group_id, sort_order,date_added) 
					values ('Set the Exclude-Basis-Weight-Fee for ems','MODULE_SHIPPING_EMS_REST_WEIGHT_UNIT_FEE', '',
					        'you can set the exclude basis weight fee here', '6', '0', now())"); 							
	  //############################for better calculate with currency#########################
	  //modified by john 2010-06-30 2/3
	  $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, 
	                                                         configuration_group_id, sort_order,date_added) 
					values ('Set the exchange rate according to CNY for EMS post','MODULE_SHIPPING_EMS_EXCHANGE_RATE', '',
					        'you can set the exchange rate here', '6', '0', now())");
	  $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, 
	                                                         configuration_group_id, sort_order, set_function, date_added) 
					values ('disabled countries for this module','MODULE_SHIPPING_EMS_DISABLED_COUNTRIES', 'DE',
	  ...
	  $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, 
	                                                         configuration_group_id, sort_order, set_function, date_added) 
					values ('enabled countries for this module','MODULE_SHIPPING_EMS_ENABLED_COUNTRIES', 'DE',
					        'enabled this shipping mothod only for those countries(use a comma separated list of the two character ISO country codes):', 
							'6', '0', 'zen_cfg_textarea(', now())");
	  
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, 
	                                                         configuration_group_id, sort_order, date_added) 
					values ('Sort Order', 'MODULE_SHIPPING_EMS_SORT_ORDER', '0', 'Sort order of display.', '6', '0', now())");
    }
    ...
    function keys() {
      $keys = array('MODULE_SHIPPING_EMS_STATUS',					
					'MODULE_SHIPPING_EMS_HANDLING_FEE',
					'MODULE_SHIPPING_EMS_BASIS_WEIGHT',					
					'MODULE_SHIPPING_EMS_BASIS_WEIGHT_FEE',
					'MODULE_SHIPPING_EMS_REST_WEIGHT_UNIT',
					'MODULE_SHIPPING_EMS_REST_WEIGHT_UNIT_FEE',
					//############################for better calculate with currency#########################
	                //modified by john 2010-06-30 3/3
					'MODULE_SHIPPING_EMS_EXCHANGE_RATE',
					'MODULE_SHIPPING_EMS_DISABLED_COUNTRIES',
					'MODULE_SHIPPING_EMS_ENABLED_COUNTRIES',
					'MODULE_SHIPPING_EMS_SORT_ORDER');
      return $keys;
    }
  }
?>